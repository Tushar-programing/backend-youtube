...
const updateAccountDetails = asyncHandler(async (req, res) => {
    ...
    const user = await User.findByIdAndUpdate(
        ...
        {
            $set: {
                fullName,   // we can write fullName: fullname but both are same so we can use only fullname
                email: email   //we can write only email beacuse both are same
            }
        },
        { new: true }
    ).select("-password")
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, user, "account detail updated sucessfuly"))
})
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    console.log("image url", coverImageLocalPath)
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        {
            $lookup: {
                from: "subscriptions", //we should use Subscription but in model all value change in lowecase as Subscription as subscription and plural as like subscription as subscriptions
                localField: "_id",
...
const getWatchHistory = asyncHandler(async (req, res) => {
    ...
    const user = await User.aggregate([
        {
            $match: {
                _id: new mongoose.Types.ObjectId(req.user._id)  // we can t use _id: req.user._id beacuse of object
            }
        },
        {
            $lookup: {
    ...
    // if (!user) {
    //     new ApiError(200, "problem in history")
    // }
